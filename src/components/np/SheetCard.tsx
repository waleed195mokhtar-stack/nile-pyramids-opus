...
import type { Sheet } from "@/hooks/useSheets";

export function SheetCard({ sheet, index = 0 }: { sheet: Sheet; index?: number }) {
  const { lang, t } = useI18n();
  const IconComp =
    ((Icons as unknown as Record<string, Icons.LucideIcon>)[sheet.icon] as Icons.LucideIcon) ??
    Icons.FileSpreadsheet;

  const title = (lang === "ar" ? sheet.title_ar : sheet.title_en) || sheet.title_en;
  const desc = (lang === "ar" ? sheet.description_ar : sheet.description_en) || "";
  ...
  return (
    ...
    >
      <GlassCard className="relative h-full overflow-hidden p-6 transition-all hover:border-[#D4AF37]/40 hover:shadow-[0_30px_60px_-20px_rgba(212,175,55,0.4)]">
        <div className="absolute -right-16 -top-16 h-40 w-40 rounded-full bg-gradient-to-br from-[#D4AF37]/20 to-transparent blur-2xl opacity-60" />
        <div className="relative">
          ...
          <div className="mt-1.5 line-clamp-2 text-sm text-white/55">{desc}</div>
